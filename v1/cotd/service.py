...
def factory(
    ...
    commands: typing.List[telegram.BotCommand],
    handlers: typing.List[HandlerGroup],
    storage: v1.cotd.storage.TelegramSavedMessagesStorage,
    static_content: StaticReader,
) -> COTDBotService:
    storage: v1.cotd.storage.TelegramSavedMessagesStorage | v1.cotd.storage.TelegramSavedMessagesStorageDev = (
        storage
        if features.feature_enable_persistence
        else v1.cotd.storage.TelegramSavedMessagesStorageDev(options.db)
    )
